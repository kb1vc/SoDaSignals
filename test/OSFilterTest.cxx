...
bool test() {
  ...
  SoDa::Checker chk(Fs, filt.getTaps(), 1.0, 35.0, 0.01, buflen, 128); 

  bool passed = true; 

  std::vector<float> gainv(chk.getNumFreqSteps());
  std::vector<float> phasev(chk.getNumFreqSteps());  
  ...
  for(uint32_t i = 0; i < chk.getNumFreqSteps(); i++) {
    ...
    if(!chk.testPassed()) {
      passed = false; 
      dumpGains(chk, chk.getFreq(i), gainv, phasev);
    }
  }
